    ...
    class FineTuner:
        ...
        def prepare_model(self, model):
            ...
            peft_config = LoraConfig(
                ...
                target_modules=[module for module, active in self.target_modules.items() if active],
                bias="none",
        ...
        def save_hyperparameters(self, output_directory):
            hyperparameters = {
                ...
                "target_modules": {k: v for k, v in self.target_modules.items()},
            }
        ...
        def adjust_training_parameters(self):
            while True:
                try:
                    ...
                    print(f"8. Precision (Current: {self.precision})")
                    print(f"9. LoRA Alpha (Current: {self.lora_alpha})")
                    print(f"10. Rank (r) (Current: {self.r})")
                    print(f"11. LoRA Dropout (Current: {self.lora_dropout})")
                    print("12. Adjust Target Modules")
                    print("13. Back to Main Menu")

                    choice = input("Select the parameter to adjust: ")

                    if choice == "1":
                        self.warmup_steps = int(input("Enter new value for Warmup Steps: "))
                    elif choice == "2":
                        self.per_device_train_batch_size = int(input("Enter new value for Per Device Train Batch Size: "))
                    elif choice == "3":
                        self.gradient_accumulation_steps = int(input("Enter new value for Gradient Accumulation Steps: "))
                    elif choice == "4":
                        self.max_steps = int(input("Enter new value for Max Steps: "))
                    elif choice == "5":
                        self.learning_rate = float(input("Enter new value for Learning Rate: "))
                    elif choice == "6":
                        self.logging_steps = int(input("Enter new value for Logging Steps: "))
                    elif choice == "7":
                        self.save_steps = int(input("Enter new value for Save Steps: "))
                    elif choice == "8":
                        self.precision = torch.float16 if input("Enter 'fp16' for half precision or 'fp32' for full precision: ") == "fp16" else torch.float32
                    elif choice == "9":
                        self.lora_alpha = int(input("Enter new value for LoRA Alpha: "))
                    elif choice == "10":
                        self.r = int(input("Enter new value for Rank (r): "))
                    elif choice == "11":
                        self.lora_dropout = float(input("Enter new value for LoRA Dropout: "))
                    elif choice == "12":
                        self.adjust_target_modules()
                    elif choice == "13":
                        break
                    else:
        ...
        def adjust_target_modules(self):
            module_descriptions = {
                "q_proj": "Query Projection: Important for determining which parts of the input sequence the model should focus on.",
                "k_proj": "Key Projection: Helps in matching queries to the relevant keys in the attention mechanism.",
                "v_proj": "Value Projection: Retrieves the actual values associated with the keys for attention.",
                "o_proj": "Output Projection: Combines the attention results and projects them to the output space.",
                "gate_proj": "Gate Projection: Used in models with gating mechanisms to control information flow.",
                "up_proj": "Up Projection: Used in models with feed-forward layers to project inputs to a higher-dimensional space.",
                "down_proj": "Down Projection: Projects from a higher-dimensional space back to the original space.",
                "lm_head": "Language Model Head: Converts the model's output to logits over the vocabulary. Typically frozen during fine-tuning.",
            }

            use_case_descriptions = {
                "q_proj": ["Tasks requiring strong focus on relevant input parts", "Memory-efficient tuning"],
                "k_proj": ["Tasks needing precise query-key matching", "Balancing memory and performance"],
                "v_proj": ["Tasks where value extraction is crucial", "General purpose tuning"],
                "o_proj": ["Tasks needing complex output projections", "Enhanced model expressiveness"],
                "gate_proj": ["Models with gating mechanisms", "Controlled information flow"],
                "up_proj": ["Feed-forward layers requiring up projection", "Tasks needing higher-dimensional processing"],
                "down_proj": ["Feed-forward layers requiring down projection", "Returning to lower-dimensional processing"],
                "lm_head": ["Tasks with custom vocabularies", "Advanced language modeling tasks"],
            }

            while True:
                print("\nCurrent Target Modules:")
                for module, active in self.target_modules.items():
                    status = "Enabled" if active else "Disabled"
                    print(f"{module}: {status} - {module_descriptions[module]}")
                    if active:
                        print(f"  Use Cases: {', '.join(use_case_descriptions[module])}")

                print("\nEnter the name of the module to toggle (e.g., 'q_proj'), or 'done' to finish:")
                choice = input()

                if choice == "done":
                    break
                elif choice in self.target_modules:
                    self.target_modules[choice] = not self.target_modules[choice]
                    status = "Enabled" if self.target_modules[choice] else "Disabled"
                    print(f"{choice} has been {status}.")
                else:
